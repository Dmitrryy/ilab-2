...
namespace ezg::engine
{
    ...
    void Core::createInstance_()
    {
        ...
        m_deletionQueue.push([inst = m_inst]()
                             {
    ...
    void Core::createLogicalDevice()
    {
        ...
        m_deletionQueue.push([dev = m_device]()
                             {
    ...
    VkFormat Core::findSupportedFormat(const std::vector< VkFormat > &candidates, VkImageTiling tiling
...
////////////////////////////////////////////////////////////////////
    ...
    void VulkanPhysicalDevices::update(const VkSurfaceKHR &surface_)
    {
        for (size_t i = 0, mi = m_devices.size(); i < mi; i++) {
            ...
            for (size_t q = 0; q < numQFamily; q++) {
                if (vkGetPhysicalDeviceSurfaceSupportKHR(curDevice, q, surface_, &(m_qSupportPresent[i][q])) !=
                    VK_SUCCESS) {
            ...
            if (vkGetPhysicalDeviceSurfaceFormatsKHR(curDevice, surface_, &numFormats, m_surfaceFormats[i].data()) !=
                VK_SUCCESS) {
                throw std::runtime_error(("failed to get physical Device surface formats!"));
            }
            ...
            printf("    API version: %d.%d.%d\n", VK_VERSION_MAJOR(apiVer), VK_VERSION_MINOR(apiVer), VK_VERSION_PATCH(
                    apiVer));
    ...
    VulkanPhysicalDevices VulkanGetPhysicalDevices(
            const VkInstance &inst_, const VkSurfaceKHR &surface_)
    {
