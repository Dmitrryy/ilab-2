...
public:
	...
	explicit cache_2Q(size_t _max_size)
		: m_hit_count(0u)
		, m_miss_count(0u)
	{
...
public:
	...
	const Val* getData() const noexcept { return m_data; }
...
Val& cache_2Q<Val, Key>::get(Key _id)
{
	...
	else 
	{
		if (m_cacheOut.contain(_id)) 
		{
			...
			const auto removed_hot = m_cacheHot.lookup(_id);
			if (removed_hot.second) {
				const auto removed_out = m_cacheOut.lookup(removed_hot.first);
				if (removed_out.second) {
					m_data[removed_out.first].reload();
				}
			}
		}
...
void cache_2Q<Val, Key>::_load_(const Key& _id)
{
	...
	{
		std::cerr << "loading error. id: " << _id << std::endl;
	}
...
template<typename Val, typename Key>
std::string cache_2Q<Val, Key>::debugString() const
{
	using std::setw;
